...
% nPower = 2;
% factorStd = 0.3;
% nInstancesEachClass = round((nPointsTrain+nPointsTest)/nClasses);
%
% X = randn(nInstancesEachClass*nClasses,nDims);
% X = bsxfun(@rdivide,X,sum(X.^2,2));
% meanRadii = repmat((1:nClasses)'.^nPower,nInstancesEachClass,1);
% stdRaddi = repmat([factorStd;factorStd*diff(unique(meanRadii))],nInstancesEachClass,1);
% radii = meanRadii+randn(size(stdRaddi)).*stdRaddi;
% Y = repmat((1:nClasses)',nInstancesEachClass,1);
% colors = {'b','r','k','c','g','y','m'};
% figure; hold on;
% for k=1:max(Y)
% plot(radii(Y==k),[colors{mod(k,numel(colors))+1} 'x']);
% end
%
% X = bsxfun(@times,radii,X);
% XTrain = X(1:nPointsTrain,:);
% XTest = X((nPointsTrain+1):end,:);
% YTrain = Y(1:nPointsTrain,:);
% YTest = Y((nPointsTrain+1):end,:);
